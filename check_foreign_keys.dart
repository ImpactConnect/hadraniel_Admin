...
void main() async {
  ...
  final dbPath = join(Platform.environment['LOCALAPPDATA']!, 'Hadraniel_Admin', 'admin_app.db');
  ...
  final outletResult = await db.query('outlets', where: 'id = ?', whereArgs: [outletId]);
  ...
  final productResult = await db.query('products', where: 'id = ?', whereArgs: [productId]);
  ...
  final outletCountResult = await db.rawQuery('SELECT COUNT(*) as count FROM outlets');
  ...
  final stockBalanceCountResult = await db.rawQuery('SELECT COUNT(*) as count FROM stock_balances');
  ...
  if (orphanedOutlets.isNotEmpty) {
    print('Missing outlet IDs: ${orphanedOutlets.map((e) => e['outlet_id']).toList()}');
  }
  
  print('Stock balances with missing products: ${orphanedProducts.length}');
  if (orphanedProducts.isNotEmpty) {
    print('Missing product IDs: ${orphanedProducts.map((e) => e['product_id']).toList()}');
  }
