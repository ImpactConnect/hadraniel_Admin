...
void main() async {
  ...
  try {
    ...
    final saleItemsResponse =
        await supabase.from('sale_items').select().limit(5);
    ...
    final saleItemsTableInfo =
        await db.rawQuery('PRAGMA table_info(sale_items)');
    ...
    final localSaleItemsCount =
        await db.rawQuery('SELECT COUNT(*) as count FROM sale_items');
    ...
    final unsyncedSales = await db
        .rawQuery('SELECT COUNT(*) as count FROM sales WHERE is_synced = 0');
    final unsyncedSaleItems = await db.rawQuery(
        'SELECT COUNT(*) as count FROM sale_items WHERE is_synced = 0');
    ...
    for (var sale in sampleSales) {
      print(
          '  Sale ID: ${sale['id']}, Total: ${sale['total_amount']}, Synced: ${sale['is_synced']}');
    }
